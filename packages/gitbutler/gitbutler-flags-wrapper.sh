...
read_flags "$XDG_CONFIG_HOME/gitbutler-flags.conf" gitbutler_flags

for flag in "${gitbutler_flags[@]}"; do
  case "$flag" in
    *=*)
...
if [[ "$(basename "$0")" == 'but' ]]; then
  exec -a but @GITBUTLER_EXEC@ "${gitbutler_args[@]}" "${args[@]}"
fi

exec @GITBUTLER_EXEC@ "${gitbutler_args[@]}" "${args[@]}"
